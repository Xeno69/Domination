...
#ifndef __DEBUG__
	private _endtime = time + (((_basetime - (([1, count (allPlayers - (entities "HeadlessClient_F"))] select isMultiplayer) * 5)) max _maxtime) + random 20);
#else
...
#ifndef __DEBUG__
	private _endtime = time + (((_basetime - (([1, count (allPlayers - (entities "HeadlessClient_F"))] select isMultiplayer) * 5)) max _maxtime) + random 40);
#else
	private _endtime = time + 10;
#endif
