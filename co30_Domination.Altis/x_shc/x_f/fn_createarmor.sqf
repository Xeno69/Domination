...
for "_nr" from 0 to 2 do {
	...
	if (_nrg > 0) then {
		...
		for "_i" from 1 to _nrg do {
			...
			__TRACE_1("","_pos")
			private _reta = [param [7], _pos, [_typenr, d_enemy_side_short] call d_fnc_getunitlistv, _newgroup, -1.111, true] call d_fnc_makevgroup;
			_newgroup deleteGroupWhenEmpty true;
			__TRACE_1("","_reta")
			d_x_sm_vec_rem_ar append (_reta # 0);
			if (random 100 < 80) then {{_x allowCrewInImmobile true;false} count (_reta # 0)};
