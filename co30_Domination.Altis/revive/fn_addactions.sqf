...
if (_unit getVariable ["xr_ReviveAction", -9999] == -9999) then {
	...
	_id = _unit addAction [format ["<t color='#FF0000'>%2 %1</t>", _unit call d_fnc_getplayername, localize "STR_DOM_MISSIONSTRING_1704"], {player setVariable ["xr_cursorTarget", _this # 0]; 0 spawn xr_fnc_drag}, [], -2, false, false, "", "_target getVariable 'xr_pluncon' && {!(_this getVariable 'xr_pisinaction') && {!(_target getVariable ['xr_dragged', false])}}", 3];
	__TRACE_1("id2","_id")
