...
if (_vec isKindOf "Air") then {
	if (isClass (configFile>>"CfgVehicles">>(typeOf _vec)>>"Components">>"TransportPylonsComponent")) then {
		_vec setVariable ["d_plyonloadoutaction", [
				...
				/* 4 condition to show */			"speed _target == 0 && {!isEngineOn _target && {d_add_resp_points_pos findIf {_x distance2D _target < 700} > -1 && {alive player && {!(player getVariable 'xr_pluncon') && {!(player getVariable ['ace_isunconscious', false]) && {player == driver _target}}}}}}",
				/* 5 condition for action */		"true",
				/* 6 code executed on start */		{},
				/* 7 code executed per tick */		{},
				/* 8 code executed on completion */	{createDialog "D_PylonLoadoutDialog"},
