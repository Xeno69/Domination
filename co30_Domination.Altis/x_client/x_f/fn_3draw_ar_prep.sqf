...
d_3draw_ar_prep = [];
{
	...
	if (_distp < 250) then {
		...
		if (_cwt == -1) then {
			...
			d_3draw_ar_prep pushBack ["#(argb,8,8,3)color(0,0,0,0)", [0, 0, 1, 1 - (_distp / 200)], ASLToAGL ((getPosASL _obj) vectorAdd [0, 0, (_x # 2) + (_distp * 0.05)]), 1, 1, 0, format [d_d3d_locs4a, round (_cwt - time)], 1, 0.033 - (_distp / 9000), "RobotoCondensed"];
		};
	};
} forEach d_3draw_ar;