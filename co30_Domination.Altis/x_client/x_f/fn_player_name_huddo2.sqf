...
if (!d_show_pname_hud && {!visibleMap && {isNil "d_is_sat_on"}}) then {
	...
	if (alive _ct && {_ct isKindOf "CAManBase" && {!(player getVariable ["xr_pluncon", false]) && {!(player getVariable ["ace_isunconscious", false]) && {_ct != player && {(positionCameraToWorld [0,0,0]) distance _ct <= (d_dist_pname_hud / 2) && {_ct getVariable ["xr_lives", 1] > -1 && {side (group _ct) getFriend d_player_side >= 0.6}}}}}}}) then {
